...
public final class StudentAnalytics {
    ...
    public String mostCommonFirstNameOfInactiveStudentsParallelStream(
            final Student[] studentArray) {
        //throw new UnsupportedOperationException();
    	Student mostCommon = null;

//    	Map<String, List<Student>> fnameCount = Stream.of(studentArray)
//    			.parallel()
//    			.collect(Collectors.groupingBy(s -> s.getFirstName()));
    	
    	

    	/*List<String> list = Stream.of(studentArray)
    			.parallel()
    			.filter(s -> !s.checkIsCurrent())
    			.map(a -> a.getFirstName()).collect(Collectors.toList());*/
    	
//    	Map<String, Long> occurrences = 
//    			Stream.of(studentArray)
//    			.parallel()
//    			.filter(s -> !s.checkIsCurrent())
//    			.map(a -> a.getFirstName()).collect(Collectors.toList()).stream().collect(Collectors.groupingBy(w -> w, Collectors.counting()));
    	
    	String max=Stream.of(studentArray)
    			.parallel()
    			.filter(s -> !s.checkIsCurrent()).parallel()
    			.map(a -> a.getFirstName()).collect(Collectors.toList()).stream()
    			.parallel()
